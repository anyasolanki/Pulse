...
def create(owner_id, story_id, story_title, call, created_at, expires_at):
    sql = '''INSERT INTO predictions
        (id, owner_id, story_id, story_title, call, created_at, expires_at)
        VALUES (%s, %s, %s, %s, %s, %s, %s)
        ON CONFLICT (owner_id, story_id) DO NOTHING
        RETURNING *'''
    try:
        with _connect() as connection, connection.cursor() as cursor:
            cursor.execute(sql, (uuid4(), owner_id, story_id, story_title, call, created_at, expires_at))
            row = cursor.fetchone()
