...
type Prediction = { id: string; story_id: number; story_title: string; call: 'yes' | 'no'; status: 'pending' | 'resolved' | 'unverifiable'; created_at: string; expires_at: string; correct: boolean | null; first_qualified_at: string | null };
...
export default function Calls({ onOpen }: { onOpen: (id: number) => void }) {
  ...
  const refresh = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch('/api/pulse/v1/predictions', { headers: localUserHeaders() });
      const data = await response.json() as { predictions?: Prediction[]; detail?: string };
      if (!response.ok) throw new Error(data.detail || 'Could not load your calls.');
      setCalls(data.predictions || []); setError('');
